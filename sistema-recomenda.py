...
# Objetivo: Implementar un sistema de recomendacion en python.
# Ejecucion del programa: python [file].py [matriz].txt
'''
...
def LeerFichero():
    ...
    for i in range(len(usuarios)):
        ...
        #print ("Usuario " + i +"Valoracion: " + usuarios[i])
...
def CoeficientePearson(usuariox, usuarioy):
    ...
    #print("La media usuariox = ", mediax)
    #print("La media usuarioy = ", mediay)
    #print("\n")
    ...
    ypow = 0.0
    for i in range(len(usuariox)):
        #numerador += vcalificacionx[i]-mediax*(vcalificaciony[i]-mediay) //unsupported operand type(s) for -: 'str' and 'float'
        numerador += i-mediax*i-mediay
    for i in range(len(usuariox)):
        #xpow = xpow + (math.pow(vcalificacionx[i]-mediax,2)) //unsupported operand type(s) for -: 'str' and 'float'
        xpow += math.pow(i-mediax,2)
    for i in range(len(usuariox)):
        #ypow = ypow + (math.pow(vcalificaciony[i]-mediay,2)) //unsupported operand type(s) for -: 'str' and 'float'
